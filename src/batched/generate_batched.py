...
mled = None #BERTUnfactoredDisambiguator.pretrained()
tagger = None#DefaultTagger(mled, 'pos')
...
class BasicGenerator:
    ...
    def generate_attn(self, input_text, max_length, solver="max", use_entropy = False, use_logprob = False):
        ...
        print("batched gen in generate_attn",time.time()-st_time)
        ...
        seqentropies_batch = []
        total_time_attn = 0
        for sample_idx,generated_tokens in enumerate(generated_tokens_all):
            ...
            # attention
            st_time = time.time()
            atten = self.model(generated_tokens_all[sample_idx:sample_idx+1], output_attentions=True).attentions[-1][0]
            atten = atten[:,:len(tokens),:len(tokens)]
...
class AttnWeightRAG(BasicRAG):
    ...
    def inference(self, question, demo, case):
        ...
        for trial in range(max_trials):
            ...
            start_time = time.time()
            new_text, _, _ = self.generator.generate_batched(prompts, self.generate_max_length) #NOTE: need modification
