...
def regenerate_answer(cot, tokenizer, model, case, demo):
    ...
    # print('>>>>>>>>>> in regenerate, text:\n', text, "\n <<<<<<<<<")
    text = cot + text.strip()
    # print('>>>>>>>>>> in regenerate after cot+, text:\n', text, "\n <<<<<<<<<")
    for word in split_words:
        pos = text.find(word)
        if pos != -1:
            text = text[:pos] 
    ...
    # print(text)
    # print("##### pred #####")
    # print('>>>>>>>>>> in regenerate after split, text:\n', text, "\n <<<<<<<<<")
    return text
...
def main():
    ...
    lines = []
    for out_file in os.listdir(args.output_dir):
        if ".txt" not in out_file: continue
        with open(os.path.join(args.output_dir, out_file), "r") as fin:
            lines += fin.readlines()
    
    # need_generate = args.dataset in ['2wikimultihopqa', "hotpotqa", "iirc", "strategyqa","arabicaqa"] 
    need_generate = False
    if need_generate:
    ...
    for line in tqdm(lines):
        ...
        if need_generate:
            ...
            # print('>>>>>>>>>> inside need_generate, pred:\n', pred, "\n <<<<<<<<<") 
        pred = data.get_real_prediction(pred)
        # print('>>>>>>>>>> after get_real_prediction, pred:\n', pred, "\n <<<<<<<<<")
        # print("*****", pred)
