...
class AraDPR:
    def __init__(
        ...
        print("Start tsv ...")
        self.inference = Inference(tsv_file_path)
        print("GET docs")
    ...
    def retrieve(
        ...
        final_result = self.inference.get_docs(queries[0],topk)
        # prepare outputs
        docids: List[str] = []
        docs: List[str] = []
        ...
        **search_engine_kwargs,
        ...
        if engine == 'elasticsearch':
            ...
            self.retriever = EvaluateRetrieval(
                BM25Search(index_name=index_name, hostname='localhost', initialize=False, number_of_shards=1,language="arabic"),
