...
class ArabicaQA(BaseDataset):
    examplars: List[Dict] = [
    {
        "question": "ما هي أوربيتالات الجزيئية التي تشكل الرابطة (3c-4e)؟ وما هي وظائفها في الترابط الجزيئي؟",
        "cot": "الرابطة (3c-4e) توصف بأنها تتكون من ثلاث أوربيتالات جزيئية. يتم تشكيل هذه الأوربيتالات عبر اتحاد مدار p في الذرة المركزية مع مدارين من الليجند. هذه الأوربيتالات تتوزع كالتالي: أوربيتال ترابط مشغول، المدار الجزيئي المشغول الأعلى (HOMO)، والمدار الجزيئي غير المشغول الأدنى (LUMO).",
        "answer": "ثلاث أوربيتالات جزيئية: أوربيتال ترابط مشغول، HOMO، وLUMO."
    },
    {
        "question": "ماذا تضمنت الحزمة الرواتب؟",
        "cot": "الحزمة تضمنت راتبًا قدره 1.4 مليون جنيه إسترليني. بالإضافة إلى ذلك، تضمنت مكافأة مالية بقيمة 2.4 مليون جنيه إسترليني ومكافأة أسهم بقيمة 6 ملايين جنيه إسترليني.",
        "answer": "1.4 مليون جنيه إسترليني كراتب، 2.4 مليون جنيه إسترليني كمكافأة مالية، و6 ملايين جنيه إسترليني كمكافأة أسهم."
    },
    {
        "question": "من بنى كنيسة سيدة العذراء وفي أي سنة؟",
        "cot": "المسؤول عن بناء كنيسة سيدة العذراء هو هنري فييل، ولم تُذكر سنة البناء ضمن البيانات المتوفرة.",
        "answer": "هنري فييل."
    },
    {
        "question": "ما هو استخدام SN 1993J في تقدير المسافة بين الأرض والمجرة M81؟",
        "cot": "تم استخدام SN 1993J، وهو مستعر أعظم، لتقدير المسافة بين الأرض والمجرة M81. المسافة المحسوبة هي 8.5 ± 1.3 مليون سنة ضوئية (2.6 ± 0.4 مليون فرسخ فلكي).",
        "answer": "8.5 ± 1.3 مليون سنة ضوئية (2.6 ± 0.4 مليون فرسخ فلكي)."
    },
    {
        "question": "ما هي المؤلفات الطبية لابن باجة؟",
        "cot": "من المؤلفات الطبية لابن باجة: كتاب \"كلام على شيء من كتاب الأدوية المفردة لجالينوس\"، كتاب \"التجربتين على أدوية ابن وافد\"، وكتاب \"اختصار الحاوي للرازي\". هذه الكتب تضمنت تحليلًا وملاحظات قيمة في مجال الطب.",
        "answer": "كلام على شيء من كتاب الأدوية المفردة لجالينوس، التجربتين على أدوية ابن وافد، اختصار الحاوي للرازي."
    },
    {
        "question": "كيف تعافت سنغافورة من الأزمة الاقتصادية في عام 2009؟",
        "cot": "سنغافورة واجهت انكماشًا اقتصاديًا وصل إلى -6.8% في الربع الرابع من عام 2009. لكنها استعادت نموها الاقتصادي في النصف الأول من عام 2010، مما يعكس تعافيًا سريعًا نتيجة السياسات الاقتصادية المناسبة.",
        "answer": "استعادت نموها الاقتصادي في النصف الأول من عام 2010."
    }
    ]

    demo_input_template = lambda self, ques: f'Question: {ques} '
    # test_input_template is for the prompt at generation time
    test_input_template = lambda self, ques: f'''Answer the following question by reasoning step-by-step if needed. You should STRICTLY answer in the following foramt and nothing else: <reasoning> reasoning </reasoning> <answer> answer </answer> Question: {ques} ''' 
    output_template = lambda self, cot, ans: f'<reasoning> {cot} </reasoning> <answer> {ans} </answer>'

    def __init__(self, data_path: str):
        logger.info(f"Loading ArabicaQA from {data_path}")
        dataset = []
        print("Loading Test ....")
        with open(os.path.join(data_path, 'test-open.json'), "r") as fin:
            js = json.load(fin)
            for example in tqdm(js):
    ...
    def get_real_prediction(self, pred):
        ...
        beg = pred.find(answer_start)
        beg = 0 if beg == -1 else beg + len(answer_start)
        end = pred.find(answer_end)
        end = len(pred) if end == -1 else end
