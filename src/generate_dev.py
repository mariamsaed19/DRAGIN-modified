...
#     "python -m sglang.launch_server --model-path CohereForAI/aya-expanse-8b --mem-fraction-static 0.7 --port 30026 --host 0.0.0.0 --max-running-requests 1024 --max-prefill-tokens 8000"
# )

# wait_for_server("http://localhost:30026")

class BasicGenerator:
    def __init__(self, model_name_or_path):
        logger.info(f"Loading model from {model_name_or_path} in dev file *******")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
        self.model_config = AutoConfig.from_pretrained(model_name_or_path,
                    trust_remote_code = "falcon" in model_name_or_path)
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map="auto", 
                    trust_remote_code = "falcon" in model_name_or_path)
        # self.vllm_model = LLM("CohereForAI/aya-expanse-8b",enable_prefix_caching=True,gpu_memory_utilization=0.9)
        self.client = openai.Client(base_url="http://127.0.0.1:30024/v1", api_key="None")

        # self.sampling_params = SamplingParams(max_tokens=1024,logprobs=20,temperature=0,top_p=0.9,top_k=25)
    ...
    def generate(self, input_text, max_length, return_logprobs=False):
        # outputs = self.vllm_model.chat([[{'role':'user','content':input_text}]],self.sampling_params)
        input_ids = self.tokenizer.encode(input_text,truncation=True, max_length=7100, return_tensors="pt")[0]
        ...
        return outputs.choices[0].message.content, None, None
    ...
    def generate_attn(self, input_text, max_length, solver="max", use_entropy = False, use_logprob = False):
        ...
        generated_tokens = self.tokenizer.encode(outputs.choices[0].message.content, return_tensors="pt").to(self.model.device) #torch.tensor([list(outputs[0].outputs[0].token_ids)]).to(self.model.device) #outputs.sequences[:, input_length:]
        ...
        if use_entropy:
            ...
            softmax_probs = np.exp(np.array(tmp))
    ...
    def __init__(self, args):
        ...
        if "retriever" in self.__dict__:
            ...
            if self.retriever_type == "BM25":
                ...
                self.retriever = AraDPR("/scratch/mariam.saeed/side/advanced-nlp/ArabicaQA/ArabicaQA/DPR/wiki/wikiAr.tsv")
...
class AttnWeightRAG(BasicRAG):
    ...
    def inference(self, question, demo, case):
        ...
        for trial in range(max_trials):
            ...
            if tokens_count > self.generate_max_length or "</answer>" in text or trial==max_trials-1:
